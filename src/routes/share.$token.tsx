...
function SharedStoryPage() {
  ...
  useEffect(() => {
    void loadSharedStory({ data: { token } }).then(setStory).catch(() => setStory(null));
  }, [token]);
    ...
    <Shell>
      ...
      {story === null ? (
        <div className="rounded-lg bg-surface p-5 shadow-[var(--shadow-border)]">
          ...
          <p className="mt-2 text-muted">This link may have expired or been revoked by its owner.</p>
        </div>
      ) : null}
      {story ? (
        <>
