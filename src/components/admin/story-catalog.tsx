...
import { createStoryShare, listStoryShares, revokeStoryShare } from "@/lib/story-api";
import { redeemPlanGift } from "@/lib/plan-gifts";
import type { EffectiveEntitlements } from "@/lib/account-tiers";
...
export function StoryCatalog({
  ...
  const [report, setReport] = useState<{ imported: string[]; skipped: { id: string; issues: string[] }[] } | null>(null);
  ...
  const [accountTier, setAccountTier] = useState(tier);
  ...
  const storyLimit = entitlements.unlimited ? "unlimited" : String(entitlements.stories ?? "—");
    ...
    <div>
      <div className="mb-6">
        ...
        <h1 className="font-display text-3xl tracking-tight">
          {role === "owner" ? "Site stories" : "Administer stories"}
        </h1>
        <p className="mt-1 max-w-2xl text-muted">
          Upload, rewrite, and manage private, unlisted, or public stories.
          {entitlements.unlimited
            ? " Owner access is not limited by a paid-plan quota."
            : ` Your current account tier is ${accountTier} (${storyLimit} stories).`}
        </p>
        {role !== "owner" ? (
          <form
