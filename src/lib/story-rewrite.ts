...
import { parseStoryBlob, validateStory } from "./story-validate.ts";
...
export type StoryRewriteOptions = {
  baseUrl: string;
  model?: string;
  apiKey?: string;
...
export async function rewriteStoryContent(
  ...
  if (content.length > 120_000) {
    return { ok: false as const, error: "Story exceeds the 120,000 character rewrite limit" };
  }
  const model = options.model?.trim() || "qwen3:4b";
...
export const rewriteStoryAsV2 = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await requireUserId();
    return rewriteStoryContent(data, {
      baseUrl: process.env.STORY_MODEL_BASE_URL || "",
      model: process.env.STORY_MODEL_NAME,
      apiKey: process.env.STORY_MODEL_API_KEY,
    });
  });
